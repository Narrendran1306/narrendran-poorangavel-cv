import React from 'react';
import { Phone, Mail, MapPin, Globe } from 'lucide-react';
import type { ContactInfo } from '../../types/portfolio.types';
import './Header.css';

interface HeaderProps {
  contact: ContactInfo;
}

// Crisp inline SVGs for LinkedIn and GitHub
const LinkedinIcon: React.FC<{ size?: number; className?: string }> = ({
  size = 13,
  className = '',
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect width="4" height="12" x="2" y="9" />
    <circle cx="4" cy="4" r="2" />
  </svg>
);

const GithubIcon: React.FC<{ size?: number; className?: string }> = ({
  size = 13,
  className = '',
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

export const Header: React.FC<HeaderProps> = ({ contact }) => {
  return (
    <header className="header-card">
      <div className="header-top-row">
        <h1 className="header-name">{contact.name}</h1>
        <div className="header-badges">
          {contact.badges.map((badge, idx) => (
            <span key={idx} className="status-badge">
              {badge}
            </span>
          ))}
        </div>
      </div>

      <div className="header-role">{contact.roleTitle}</div>

      <div className="header-contact-row">
        <a
          href={`tel:${contact.phone.replace(/\s+/g, '')}`}
          target="_blank"
          rel="noopener noreferrer"
          className="contact-link"
          title={`Call ${contact.phone}`}
        >
          <Phone className="contact-icon" size={13} />
          <span>{contact.phone}</span>
        </a>
        <a
          href={`mailto:${contact.email}`}
          target="_blank"
          rel="noopener noreferrer"
          className="contact-link"
          title={`Email ${contact.email}`}
        >
          <Mail className="contact-icon" size={13} />
          <span>{contact.email}</span>
        </a>
        <div className="contact-item">
          <MapPin className="contact-icon" size={13} />
          <span>{contact.location}</span>
        </div>
      </div>

      <div className="header-links-row">
        <a
          href={contact.linkedin.url}
          target="_blank"
          rel="noopener noreferrer"
          className="contact-link social-link"
          title="Open LinkedIn profile in new tab"
        >
          <LinkedinIcon className="contact-icon" size={13} />
          <span>{contact.linkedin.label}</span>
        </a>
        <a
          href={contact.github.url}
          target="_blank"
          rel="noopener noreferrer"
          className="contact-link social-link"
          title="Open GitHub profile in new tab"
        >
          <GithubIcon className="contact-icon" size={13} />
          <span>{contact.github.label}</span>
        </a>
        <a
          href={contact.portfolio.url}
          target="_blank"
          rel="noopener noreferrer"
          className="contact-link social-link"
          title="Open live Portfolio in new tab"
        >
          <Globe className="contact-icon" size={13} />
          <span>{contact.portfolio.label}</span>
        </a>
      </div>
    </header>
  );
};

export default Header;
